...
> field-weighted, prefix + one-typo tolerance, accent folding) with `q`,
> `phase`, `region`, `year`, `type` and `sort` in the query string, per-option
...
> there is no topic facet (topics are a whole-collection synthesis, not a
> project tag), project type replaced publication status, and matched words are
> not yet highlighted.
...
**From the first real export (2026-09-14, four responses)** — now wired end to end
via `scripts/import-survey.mjs`; these are what the demo raised:

- **Topic areas are a synthesis, not a project tag** (decided September 2026). An LLM
  — Google NotebookLM, per Stephanie — synthesizes needs across all collected findings;
  the result goes into `topicSummaries.json` after review. Projects carry no `topics`
  field, the explorer has no topic filter, and topic pages no longer list projects.
  The notebook's source document is generated by `scripts/notebooklm-source.mjs`
  (run by the importer); workflow and prompt are in `notebooklm/README.md`. This
  replaces the Gemini-API `synthesize-needs.mjs` idea in §3.2.
- **Is there an "After" fire cycle phase?** The export only exercised Before, During
  and Long-Term. The importer warns on any other value.
- **Author order.** Currently submitter (Q1) first, then Q6 in the order typed. The
  NOAA tech memo lists Emily last, so this may not match how they want to be cited.
  Names with more than two words ("Laura Sample McMeeking") are kept verbatim rather
  than guessing a surname — the DOI record confirms "Sample McMeeking" is the family name.
- **Publication status** is not asked; the badge is hidden unless an editor sets it.
- **Entry formatting.** One response pasted two needs into one box (auto-split), and
  two entries omit the colon after "Need for … among …", so their lead can't be bolded.
  Worth a note in the survey instructions.
- **Testbed #002 and #003 cite the same tech memo DOI**, which covers #002–004. Fine,
  but the citation block uses it as the locator for both.

**For Emily and Steph** (the mapped survey-question list covers most of this):

1. ~~**Fire cycle phase — what are the exact values?**~~ **Answered** by the export:
   Before / During / Long-Term (see above re: "After"). They become a fixed union type
