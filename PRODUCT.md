...
**Shipped capabilities:** landing (hero, GACC coverage map, live stats, topic cards, submit band); project explorer with search, topic/phase/GACC/year/status filters, sort, and detail modal; per-topic pages; light/dark theme; content adapters (`local` default, Sanity and Strapi stubs).
